...
def assemble_table(conn, table_name):
    files = os.listdir("integration_new/test_files/sql_plain/")
    first_partition_flag = True
    for table_path in files:
        if table_path == f"{table_name}0.csv" or table_path == f"{table_name}1.csv":
            if first_partition_flag:
                query = f"CREATE TABLE {table_name} AS SELECT * FROM " \
                        f"'integration_new/test_files/sql_plain/{table_path}'"
                conn.execute(query)
                first_partition_flag = False
            else:
                query = f"INSERT INTO {table_name} SELECT * FROM " \
                        f"'integration_new/test_files/sql_plain/{table_path}'"
                conn.execute(query)
...
def assemble_orders(conn):
    files = os.listdir("integration_new/test_files/sql_plain/")
    for table_path in files:
        if table_path == f"orders0.csv":
            print("Creating o1")
            query = f"CREATE TABLE o1 AS SELECT * FROM read_csv('integration_new/test_files/sql_plain/{table_path}', " \
                    f"ignore_errors=1, auto_detect=1)"
            conn.execute(query)
            first_table_flag = False
        if table_path == f"orders1.csv":
            print("Creating o2")
            query = f"CREATE TABLE o2 AS SELECT * FROM read_csv('integration_new/test_files/sql_plain/{table_path}', " \
                    f"ignore_errors=1, auto_detect=1)"
            conn.execute(query)
...
#     with open(f"numbers/tpch_base/{data_dir}_2.csv", "a") as file:
